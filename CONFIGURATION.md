...
| YouTube | `yt-dlp` CLI installed; `SCRAPECREATORS_API_KEY` adds a server-side transcript fallback used only when yt-dlp fails (429 / bot-gate) plus a search backfill when yt-dlp returns fewer than 3 videos | always on if `yt-dlp` present; SC transcript fallback default-on when key set (no credit spent unless yt-dlp fails); SC search backfill default-on below 3 videos (~1 search per thin subquery) | yes |
